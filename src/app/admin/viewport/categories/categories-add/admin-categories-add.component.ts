...
export class AdminCategoriesAddComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.categoryForm = this.fb.group({
      ...
      catSlug: [null, [Validators.required]],
      catBrief: null,
  ...
  onLangChanges() {
    ...
    this.title.setTitle(this.translateSer.instant('add-admin-categories-page.page-title'));
  ...
  // save new category on form submit
  saveNewCategory(form: FormGroup) {
    const values = form.getRawValue();
    const children = values.children.map((child: Category) => {
      return {
        catName: child[Object.keys(child)[0]],
        catSlug: child[Object.keys(child)[1]]
      };
    });
    const newObject: Category = {
      seqNo: Date.now(),
      catName: { [this.currentLang]: this.globalJs.toTitleCase(values.catName) },
      catSlug: values.catSlug.replace(/\s+/g, '-').toLowerCase(),
      catBrief: { [this.currentLang]: values.catBrief },
    ...
    this.categoriesSer.addNewCategory(newObject)
      .then(() => {
        this.catImagesValueChanged = false;
        this.mainLoadingSer.endLoading();
        this.notifySer.setNotify({class: 'success', msg: this.translateSer.instant('toast-notifications.category-saved'), time: 5000});
        form.reset();
      ...
      .catch(() => {
        this.mainLoadingSer.endLoading();
        this.router.navigate(['page/1'], { relativeTo: this.activeRoute.parent });
        this.notifySer.setNotify({class: 'danger', msg: this.translateSer.instant('toast-notifications.failed-category-exist'), time: 5000});
      });
  }
